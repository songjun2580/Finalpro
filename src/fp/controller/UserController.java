...
public class UserController {
	...
	/**청소견적서 신청 주소 팝업 이동 관련 메서드*/
    @RequestMapping("/cleanJusoPopup.do")
       public String cleanJusoPopup() {
       return "user/jusoPopup";
    }
    /**청소견적서 작성 관련 메서드*/
    @RequestMapping("userCleanEstSubmit.do")//청소 견적서 작성 메서드
    public ModelAndView userCleanEstimateWrite(CleanEstimateDTO dto) {
    
       int result = cleanEstimateDao.insertCleanEst(dto);
       
       String msg=result>0?"견적서 제출 완료":"견적서 제출 실패";
	...
	public ModelAndView userCompWrite(@RequestParam(value="moIdx" , defaultValue = "0") int moIdx) {
		  ModelAndView mav= new ModelAndView(); 
			MoveEstimateDTO dto= moveEstimateDao.moveEstimate(moIdx);
			ComInfoDTO dto2= blogContentDao.getCoInfo(dto.getCoIdx());
			mav.addObject("dto2", dto2);
		    mav.addObject("dto", dto); 
		    mav.setViewName("user/userCompWrite"); 	
		      return mav;
	}
	...
	public ModelAndView userCompWriteSubmit(@RequestParam(value = "cplImgs" ,required = false ) List<MultipartFile> file , 
		...
		UploadFileUtils upLoad = new UploadFileUtils();
	         ...
	         dto.setCplImg(dbName);
	         int count=cplDao.cplWrite(dto);
	         System.out.println("a");
	         int result=cplDao.coCplUpdate(dto.getCoIdx());
	...
	public String userMoveList(
	  ...
	  /**결제내역 이동*/
	   @RequestMapping("/endPriceList.do")
	   public String endPriceList(HttpSession session) {
	      String uIdx_s=session.getAttribute("uIdx").toString();
	      if(uIdx_s==null||uIdx_s.equals("")) {
	         uIdx_s="0";
	      }
	      int uIdx=Integer.parseInt(uIdx_s);
	      List endlist=userInfoDao.endPriceList(uIdx);
	      session.setAttribute("endlist", endlist);
	      System.out.println("엔드리스트"+endlist);
	      return "user/endPriceList";
	   }

	   /**리뷰작성 페이지 이동 관련 메서드*/
		@RequestMapping("/userReviewWrite.do")//리뷰작성 페이지 이동 관련 메서드
		   public ModelAndView userReviewWrite(@RequestParam(value="moIdx" , defaultValue = "0") int moIdx) {
		    ModelAndView mav= new ModelAndView(); 
			MoveEstimateDTO dto= moveEstimateDao.moveEstimate(moIdx);
			ComInfoDTO dto2= blogContentDao.getCoInfo(dto.getCoIdx());
			mav.addObject("dto2", dto2);
		    mav.addObject("dto", dto); 
		    mav.setViewName("user/userReviewWrite"); 	
		      return mav;
		   }
	
	/**리뷰작성 관련 메서드*/
	   @RequestMapping(value="userReviewWriteSubmit.do",method = RequestMethod.POST)
	  ...
	  /**이메일 인증번호 전송 관련 메소드 */
	   ...
	   public ModelAndView emailNumAuth(
		   ...
		   if(userkey.equals(key)) {
			   msg="<span style='color:blue'>인증번호가 일치합니다.</span><input type='hidden' id='Email_check' value='1'>";
		   }else {
			   msg="<span style='color:red'>인증번호가 일치하지 않습니다.</span><input type='hidden' id='Email_check' value='1'>";
		   }
		   mav.addObject("msg",msg);
   ...
   public ModelAndView studentSubmit() {
	...
	/**이사 견적서 조회 관련 메서드*/
	   ...
	   public ModelAndView payInsert(int moconfirmPrice, int moIdx) {
	      
	      int result=userInfoDao.payInsert(moconfirmPrice);
	      ModelAndView mav=new ModelAndView();
	      mav.addObject("result",result);
	      String url = "redirect:/final2Update.do?moIdx="+moIdx;
	      return new ModelAndView(url);
	      
	   }
	   /**final 업데이트*/
	   @RequestMapping("/final2Update.do")
	   public ModelAndView final2Update(
	         @RequestParam("moIdx") int moIdx
	         ) {
	      
	      userInfoDao.final2Update(moIdx);
	      ModelAndView mav=new ModelAndView();
	      String url = "redirect:/endPriceList.do";
	      return new ModelAndView(url);
	   }
