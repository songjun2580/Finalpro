	  ...
	  <div>
		<table border="0" cellspacing="0" style="margin:0px auto; width:1400px; background-color:#E0F8F1;" >
			<thead>
				...
				<tfoot>
					<tr>
						<td style="color:red; text-align:right; font-size: 15px" colspan="8">(입찰은 선착순으로 마감되오니, 빠른 입찰 부탁드립니다. 또한, 이미 확정된 견적서는 입찰불가로 변경됩니다.)</td>
					</tr>
				</tfoot>
				<tbody>
				<c:if test="${!empty list2}">
					<c:forEach var="dto2" items="${list2}">
						<tr>
							...
							<td>
								<c:if test="${dto2.moconfirmPrice==0 }">
								<button type="button" class="btn btn-danger" onclick="javascript:bidPopup(${dto2.moIdx});">입 찰</button>
								</c:if>
								<c:if test="${dto2.moconfirmPrice!=0 }">
								<button type="button" class="btn btn-danger" onclick="javascript:bidPopup(${dto2.moIdx});" disabled="disabled">입 찰 불 가</button>
								</c:if>
							</td>
						</tr>
					</c:forEach>
